...
#define MAX_NODOS 200 //tope máximo de nodos; puedes aumentarlo si tu instancia es más grande.

double costoRuta(int ruta[], double dist[][MAX_NODOS + 1], int n);//declara la función costoRuta que usamos después del main. Observa que la matriz dist acepta índices 1..MAX_NODOS.

int main(int argc, char *argv[]) {
    const char *filename = "bayg29.tsp";//usa bayg29.tsp por defecto, o un archivo pasado como argumento ./prog archivo.tsp.
    if (argc >= 2) filename = argv[1];

    FILE *f = fopen(filename, "r");//— abre el archivo .tsp; si falla, imprime error y sale.
    if (!f) {
        perror(filename);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {//lee línea por línea.O
